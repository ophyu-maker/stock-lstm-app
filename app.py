...
with tab_pred:
    ...
    if hist_last5.empty:
     ...
     # ---- Build layered Altair chart (simple, with string dates) ----
        ...
        # ============================================================
        # EXTRA: FLEXIBLE HORIZON FORECAST (1–21 DAYS) USING 5-DAY MODEL
        # ============================================================
        st.markdown("### Extra: derived forecast over 1 day / 1 week / 1 month")

        extra_horizon_days = st.slider(
            "Choose forecast horizon in days "
            "(e.g., 1 = 1 day, 5 ≈ 1 trading week, 21 ≈ 1 trading month)",
            min_value=1,
            max_value=21,
            value=5,
        )

        # Use the average daily log-return implied by your 5-day model
        extra_dates = [
        ...
        extra_df = pd.DataFrame(
            {
                "date": [pd.to_datetime(d).normalize() for d in extra_dates],
                "price": extra_prices,
            }
        )
        ...
        st.caption(
            "This chart extends the 5-day LSTM forecast to a custom horizon by "
            "assuming the same average daily log return. "
            "For example, 1 day, 5 days (~1 week), or 21 days (~1 month)."
        )
